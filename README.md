...
The center region $R = \{c |c$ is a centerpoint $\}$. In other words, the center region is the region that contains only the centerpoints.
...
3. There are many more efficient algorithms to find the centerpoint. Our program only shows the center region via the method described in the proof for the existence of a centerpoint.